<!-- ====== BANNER (Reliable) ====== -->
<p align="center">
  <img
    src="banner.png"
    alt="Banner"
  />
</p>

<!-- ====== TYPING INTRO ====== -->
<p align="center">
  <img
    src="https://readme-typing-svg.demolab.com?font=Inter&size=18&pause=1000&center=true&vCenter=true&width=900&lines=AI+%26+Machine+Learning+Engineer+%7C+Data+Analyst;Computer+Vision+(YOLO,+OpenCV)+%7C+NLP+%7C+Deep+Learning;GenAI:+RAG+%7C+LangChain+%7C+Hugging+Face+%7C+FAISS;Building+practical,+scalable+AI+solutions"
    alt="Typing Intro"
  />
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/Vijayakumar-K">LinkedIn</a> •
  <a href="mailto:vijayakumar020206@gmail.com">Email</a> •
  <a href="https://github.com/Vijayakumar226">GitHub</a>
</p>

---

## About Me
B.Tech Artificial Intelligence and Data Science student with hands-on experience in **Machine Learning, Deep Learning, and Generative AI**. I build real-world AI applications—**computer vision systems**, **RAG-based assistants**, and **automation workflows**.

- 📍 Chennai, India  
- 🎯 Interests: ML Engineering • NLP • Computer Vision • RAG Systems • Automation  
- 🚀 Goal: Build practical and scalable AI solutions  

---

## Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-111827?style=for-the-badge&logo=python&logoColor=FFD43B)
![SQL](https://img.shields.io/badge/SQL-111827?style=for-the-badge&logo=postgresql&logoColor=336791)
![HTML](https://img.shields.io/badge/HTML5-111827?style=for-the-badge&logo=html5&logoColor=E34F26)
![CSS](https://img.shields.io/badge/CSS3-111827?style=for-the-badge&logo=css3&logoColor=1572B6)
![JavaScript](https://img.shields.io/badge/JavaScript%20(Basics)-111827?style=for-the-badge&logo=javascript&logoColor=F7DF1E)

### ML / DL / Data
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-111827?style=for-the-badge)
![Deep Learning](https://img.shields.io/badge/Deep%20Learning-111827?style=for-the-badge)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-111827?style=for-the-badge&logo=scikitlearn&logoColor=F7931E)
![TensorFlow](https://img.shields.io/badge/TensorFlow-111827?style=for-the-badge&logo=tensorflow&logoColor=FF6F00)
![PyTorch](https://img.shields.io/badge/PyTorch-111827?style=for-the-badge&logo=pytorch&logoColor=EE4C2C)
![Pandas](https://img.shields.io/badge/Pandas-111827?style=for-the-badge&logo=pandas&logoColor=150458)
![NumPy](https://img.shields.io/badge/NumPy-111827?style=for-the-badge&logo=numpy&logoColor=013243)
![Matplotlib](https://img.shields.io/badge/Matplotlib-111827?style=for-the-badge)

### GenAI / LLM Tools
![RAG](https://img.shields.io/badge/RAG-111827?style=for-the-badge)
![LangChain](https://img.shields.io/badge/LangChain-111827?style=for-the-badge)
![HuggingFace](https://img.shields.io/badge/Hugging%20Face-111827?style=for-the-badge&logo=huggingface&logoColor=FFD21E)
![FAISS](https://img.shields.io/badge/FAISS-111827?style=for-the-badge)
![Prompt Engineering](https://img.shields.io/badge/Prompt%20Engineering-111827?style=for-the-badge)

### Backend / Tools
![FastAPI](https://img.shields.io/badge/FastAPI-111827?style=for-the-badge&logo=fastapi&logoColor=009688)
![Git](https://img.shields.io/badge/Git-111827?style=for-the-badge&logo=git&logoColor=F05032)
![GitHub](https://img.shields.io/badge/GitHub-111827?style=for-the-badge&logo=github&logoColor=FFFFFF)
![VS Code](https://img.shields.io/badge/VS%20Code-111827?style=for-the-badge&logo=visualstudiocode&logoColor=007ACC)
![n8n](https://img.shields.io/badge/n8n-111827?style=for-the-badge&logo=n8n&logoColor=FF6A00)
![Power BI](https://img.shields.io/badge/Power%20BI-111827?style=for-the-badge&logo=powerbi&logoColor=F2C811)

---

## Featured Projects

### 1) Real-Time Defect Classification in Manufacturing (YOLO + FastAPI)
- Built a YOLO-based defect detection model for manufacturing images and deployed it for real-time inference.  
- Implemented a FastAPI application to accept image uploads and return predictions quickly.  
- Focused on production-style workflow: preprocessing → inference → response API.  
Repo: https://github.com/Vijayakumar226/Real-Time-Defect-Classification-in-Manufacturing-Using-Machine-Learning-and-Image-Processing

### 2) Voice-Enabled Intelligent Banking Assistant (RAG + LLMs)
- Developed a voice-enabled assistant that answers using document-grounded retrieval for higher reliability.  
- Pipeline: speech-to-text → embeddings → FAISS retrieval → LLM response → text-to-speech.  
- Designed to reduce hallucinations by generating answers strictly from retrieved context.  
Repo: https://github.com/Vijayakumar226/Voice-Enabled-Intelligent-Banking-Assistant-using-RAG-and-Large-Language-Models

### 3) AI-Powered HR Resume Screening Automation (n8n)
- Automated resume parsing and role-based skill scoring to classify candidates at scale.  
- Integrated Google Sheets for tracking and Gmail API for automated communication workflows.  
- Built a practical automation pipeline for faster HR shortlisting and reporting.  
Repo: https://github.com/Vijayakumar226/AI-Powered-HR-Resume-Screening-Automation-using-n8n

### 4) Netflix Data Analysis Dashboard (Power BI) 
- Cleaning + transformation using Power Query with data quality checks.  
- Interactive dashboard for trends by genre, country, rating, and release year.  
- Repository will be added after final polishing.  

---

## Currently Learning
- Advanced Deep Learning Architectures  
- AI Agents & Automation  
- Scalable ML Deployment and MLOps  

---

## GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Vijayakumar226&show_icons=true" height="165" alt="GitHub Stats" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Vijayakumar226&layout=compact" height="165" alt="Top Languages" />
</p>

---

## Connect With Me
- LinkedIn: https://www.linkedin.com/in/Vijayakumar-K  
- Email: vijayakumar020206@gmail.com  

⭐ Open to opportunities in **AI/ML, Data Science, and GenAI** roles.
